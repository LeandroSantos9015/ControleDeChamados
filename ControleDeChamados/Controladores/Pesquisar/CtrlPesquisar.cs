...
using Interfaces;
using GerenciadorCasaDeCarnes.Pesquisar;
//using Modelos.Cadastro;
//using Modelos.Cadastros.ParceiroNegocio;
using Customizados;
using Interfaces.PesquisarDTO;

namespace Controladores.Pesquisar
{
    public class CtrlPesquisar
    {
        ...
        public CtrlPesquisar(ITelaPrincipal Pai, IList<Object> ListaGenerica, int tamanhoDaTela, string tituloTela)
        {
            Pesquisa = new BuscaGenerica();
        ...
        private void TextoPesquisar(IList<Object> lista, string texto)
        {
            ...
            ListaTratada = lista.Cast<IConsultaPesquisar>().ToList().Where(x => x.Consulta.ToUpper().Contains(texto.ToUpper())).ToList().Cast<Object>().ToList();
